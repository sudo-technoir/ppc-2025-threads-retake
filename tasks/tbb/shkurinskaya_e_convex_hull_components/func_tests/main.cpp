...
using shkurinskaya_e_convex_hull_components_tbb::ConvexHullTbb;
using shkurinskaya_e_convex_hull_components_tbb::Point;
...
std::vector<Point> RunHull(const std::vector<uint8_t>& img, int w, int h) {
  ...
  auto task = std::make_shared<ConvexHullTbb>(td);
...
TEST(shkurinskaya_e_convex_hull_components_tbb, hull_on_solid_square_5x5) {
  const int w = 5;
  const int h = 5;
...
TEST(shkurinskaya_e_convex_hull_components_tbb, hull_on_perfect_diagonal_collinear) {
  const int w = 7;
  const int h = 7;
  std::vector<uint8_t> img(static_cast<std::size_t>(w) * static_cast<std::size_t>(h), 0);
